...
class TestHybridMetaRoundTrip:
    ...
    def _write_hybrid_npz(self, path: Path, *,
                          affine_map: Any) -> None:
        """Write a minimal REV01 hybrid NPZ via the recorder's actual
        writer so the test exercises the same serialisation path the
        runtime depends on."""
        import harmony_free_arm_calibration as recorder
        from harmony_free_arm_calibration import CaptureBundle, write_npz

        # Drive the writer down the "vlm_depth_pro anchor" branch so it
        # stamps meta["depth_source"] with the hybrid literal.
        import config as _config
        original = getattr(_config, "GAZE_CALIBRATION_DEPTH_SOURCE", "vergence")
        original_host = getattr(_config, "VLM_SERVICE_HOST", "")
        _config.GAZE_CALIBRATION_DEPTH_SOURCE = "vlm_depth_pro"
        _config.VLM_SERVICE_HOST = "192.168.99.99"
        try:
            anchor = CaptureBundle(
                t=0.0, q=np.zeros(7), ee_mm=np.zeros(3),
                gaze_x_norm=0.5, gaze_y_norm=0.5, gaze_conf=1.0,
                depth_cm=55.0, depth_valid=True, miss_mm=2.0, ipd_mm=63.0,
                imu_w=0.0, imu_fresh=True,
                head_yaw_deg=0.0, head_pitch_deg=0.0,
                gaze_yaw_deg=0.0, gaze_pitch_deg=0.0,
                phase="captured", target_label="mid_R3",
                depth_source="vlm_depth_pro",
                depth_cm_vergence=70.0,
            )
            transit = CaptureBundle(
                t=1.0, q=np.zeros(7), ee_mm=np.zeros(3),
                gaze_x_norm=0.5, gaze_y_norm=0.5, gaze_conf=1.0,
                depth_cm=72.0, depth_valid=True, miss_mm=2.0, ipd_mm=63.0,
                imu_w=0.0, imu_fresh=True,
                head_yaw_deg=0.0, head_pitch_deg=0.0,
                gaze_yaw_deg=0.0, gaze_pitch_deg=0.0,
                phase="transit", target_label="",
                leg_label="transit_a_to_b",
                depth_source="vergence",
                depth_cm_vergence=float("nan"),
            )
            write_npz([anchor, transit], str(path))
            # The recorder writes affine_map=None; if the caller wants a
            # populated dict, we post-edit the file by re-saving with the
            # updated meta. This mirrors the offline fit script's
            # behaviour (Step 7) without depending on that script.
            if affine_map is not None:
                z = np.load(str(path), allow_pickle=True)
                fields = {k: z[k] for k in z.files if k != "meta"}
                meta = z["meta"].item()
                meta["affine_map"] = affine_map
                fields["meta"] = meta
                z.close()
                np.savez_compressed(str(path), **fields)
        finally:
            _config.GAZE_CALIBRATION_DEPTH_SOURCE = original
            _config.VLM_SERVICE_HOST = original_host

    def test_hybrid_meta_string_round_trips_from_disk(self, tmp_path: Path):
        p = tmp_path / "rev01_pre_fit.npz"
        self._write_hybrid_npz(p, affine_map=None)
        # Load from disk (NOT just inspect the in-memory meta dict).
        z = np.load(str(p), allow_pickle=True)
        meta = z["meta"].item()
        assert isinstance(meta, dict)
        assert meta["depth_source"] == "hybrid_anchor_vlm_transit_vergence"
        # affine_map placeholder survives as None (not numpy scalar / array).
        assert meta["affine_map"] is None

    def test_populated_affine_map_round_trips_from_disk(self, tmp_path: Path):
        p = tmp_path / "rev01_post_fit.npz"
        self._write_hybrid_npz(p, affine_map={
            "a": 1.2, "b": -8.5, "R2": 0.92, "max_abs_residual_cm": 4.1,
        })
        # Load from disk: the nested affine_map dict must come back as a
        # real Python dict (not a 0-d object array) so callers can read
        # (a, b) directly.
        z = np.load(str(p), allow_pickle=True)
        meta = z["meta"].item()
        assert isinstance(meta, dict)
        ...
        assert am["a"] == pytest.approx(1.2)
        assert am["b"] == pytest.approx(-8.5)
        assert am["R2"] == pytest.approx(0.92)
        assert am["max_abs_residual_cm"] == pytest.approx(4.1)
