...
The bridge now publishes TWO outlets so XDF consumers can disentangle
continuous gaze samples from variable-rate eye events:

  1. `NeonGaze` — 25-channel float32, nominal 30 Hz. Per-sample gaze
     ...
     loaded by the same column-name lookup. Channels 5-24 are new
...
depth_cm is recomputed in this process (was previously sourced from
GazeSystem.get_snapshot()) using the same
...
# Channels 5-24 are pure additions; downstream code should read by
...
def _gaze_loop(
    ...
    """Worker thread: pull gaze datums, push 25-channel samples to LSL.
