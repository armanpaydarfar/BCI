...
class SerialWriter:
  ...
  def write(self, number):
    return self.ser.write( str.encode(chr(number)) )

  def json_write(self, array):
    return self.ser.write( str.encode(json.dumps({'data': array})) )
...
if __name__ == '__main__':
  ...
  while True:
    try:
      data = int( input('Input? ').strip() )
      assert 0 <= data <= 255
      sw.write(data)
      # sw.json_write( [int(x) for x in input('Input? ').split()] )
      print('Sent', data)
