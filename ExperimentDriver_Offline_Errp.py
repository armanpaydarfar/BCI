...
from Utils.experiment_utils import generate_trial_sequence_errp, display_multiple_messages_with_udp
...
def show_feedback(duration=5, mode=0):
    ...
    """
    logger.log_event(f"Feedback display started — Mode: {'MI' if mode == 0 else "REST" if mode == 1 else "ERRP"}, Duration: {duration}s")
...
trial_sequence = generate_trial_sequence_errp(config.TOTAL_TRIALS_ERRP, config.MAX_REPEATS)
logger.log_event(f"Trial Sequence: {trial_sequence}")
mode_labels = ["MI" if t == 0 else "REST" if t == 1 else "ERRP" for t in trial_sequence]
...
while running and current_trial < len(trial_sequence):
    ...
    if backdoor_mode is not None:
        mode = backdoor_mode
        logger.log_event(f"Backdoor override used: {'MI' if mode == 0 else "REST" if mode == 1 else "ERRP"}")
    else:
        mode = trial_sequence[current_trial]

    logger.log_event(f"Starting trial {current_trial+1}/{len(trial_sequence)} — Mode: {'MI' if mode == 0 else "REST" if mode == 1 else "ERRP"}")
