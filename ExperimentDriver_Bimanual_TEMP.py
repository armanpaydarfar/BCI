...
def perform_master_positioning(
    ...
    send_udp_message(
        ...
        config.ROBOT_OPCODES["MASTER_UNLOCK"],   # from config dict
        logger=logger
    )
    send_udp_message(
        udp_socket_marker,
        config.UDP_MARKER["IP"], config.UDP_MARKER["PORT"],
        config.TRIGGERS["MASTER_UNLOCK"],        # from config dict
        logger=logger
    )
    ...
    send_udp_message(
        udp_socket_robot,
        config.UDP_ROBOT["IP"], config.UDP_ROBOT["PORT"],
        config.ROBOT_OPCODES["MASTER_LOCK"],     # from config dict
        logger=logger
    )
    send_udp_message(
        udp_socket_marker,
        config.UDP_MARKER["IP"], config.UDP_MARKER["PORT"],
        config.TRIGGERS["MASTER_LOCK"],          # from config dict
        logger=logger
    )
...
def main():
    ...
    while running and current_trial < len(trial_sequence):
        ...
        # --- Robot HOME + reset for MI trials (manual, not timing-based) ---
        if mode == 0:
            ...
            # (Optional but recommended) mark robot phase end for alignment
            send_udp_message(
                udp_socket_marker,
                config.UDP_MARKER["IP"], config.UDP_MARKER["PORT"],
                config.TRIGGERS["ROBOT_END"],
                logger=logger
            )

            # Run the same 3s reset fixation you previously did only for MI-correct
            display_fixation_period(duration=3, eeg_state=eeg_state)
