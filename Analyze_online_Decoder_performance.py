...
plot_style = "dot_std"  # options: "box", "dot_std", "dot_ci", "violin", "ecdf", "forest_runwise"

RUNWISE = True           # If True: single grouped figure with per-run groupings for any plot style
...
DOT_STD_USE_SE        = True   # if False, use SD instead of SE for the vertical line(s)
...
lean_records = []  # rows: {GlobalTrialID, RunID, Class, LeanPct, Metric}
...
for gtid, trial_df in df_vis.groupby("GlobalTrialID"):
    ...
    # print(y_use, y_use.size, lean_pct, cls)

lean_df = pd.DataFrame(lean_records)


COL_MI, COL_REST = "tab:orange", "tab:blue"
RUN_COLORS = plt.rcParams['axes.prop_cycle'].by_key().get('color', ['C0','C1','C2','C3','C4','C5','C6','C7','C8','C9'])
...
# ---- Run ordering + helpers
runs_sorted = sorted(
    lean_df["RunID"].unique(),
    key=lambda r: (int(r.split("_")[-1]) if r.split("_")[-1].isdigit() else 10**9, r)
)
...
def plot_box_total(mi_vals, rest_vals, title_suffix=""):
    fig, ax = plt.subplots()
    box = ax.boxplot([mi_vals, rest_vals], labels=["MI", "REST"], widths=0.5,
                     patch_artist=True, showfliers=False)
    for patch, c in zip(box["boxes"], [COL_MI, COL_REST]):
        patch.set_facecolor(c); patch.set_alpha(0.45)
    for median in box["medians"]:
        median.set_linewidth(2)
    ax.scatter(jittered_x(1.0, len(mi_vals)), mi_vals, s=30, alpha=0.9,
               edgecolor="black", linewidth=0.5, color=COL_MI)
    ax.scatter(jittered_x(2.0, len(rest_vals)), rest_vals, s=30, alpha=0.9,
               edgecolor="black", linewidth=0.5, color=COL_REST)
    ax.set_ylim(0, 100); ax.set_ylabel("Lean %")
...
def plot_dot_std_total(mi_vals, rest_vals, title_suffix=""):
    ...
    for x, m, dn, up, col in [(0, m_mi, dn_mi, up_mi, COL_MI),
    ...
    if DOT_CI_SHOW_DOTS:
        ax.scatter(jittered_x(0, len(mi_vals)), mi_vals, s=24, alpha=0.65,
                   edgecolor="k", linewidth=0.3, color=COL_MI)
        ax.scatter(jittered_x(1, len(rest_vals)), rest_vals, s=24, alpha=0.65,
                   edgecolor="k", linewidth=0.3, color=COL_REST)

    ax.set_xticks([0, 1]); ax.set_xticklabels(["MI", "REST"])
    ax.set_ylim(0, 100); ax.set_ylabel("Lean %")
    err_label = "SE" if DOT_STD_USE_SE else "SD"
    asym = "asym" if DOT_STD_ASYMMETRIC_SE else "sym"
    ax.set_title(f"Lean% — Mean • ±{err_label} ({asym}) ({METRIC_MODE}) {title_suffix}")
    ax.grid(True, axis="y", linestyle="--", alpha=0.4)
...
def plot_dot_ci_total(mi_vals, rest_vals, title_suffix=""):
    ...
    rest_mean, (rest_lo, rest_hi) = mean_ci(rest_vals, n_boot=1000, alpha=0.05)
    if DOT_CI_SHOW_DOTS:
        ax.scatter(jittered_x(0, len(mi_vals)), mi_vals, s=24, alpha=0.5,
                   edgecolor="k", linewidth=0.3, color=COL_MI)
        ax.scatter(jittered_x(1, len(rest_vals)), rest_vals, s=24, alpha=0.5,
                   edgecolor="k", linewidth=0.3, color=COL_REST)
    ...
    ax.vlines(1, rest_lo, rest_hi, linewidth=3, color=COL_REST)
    ax.set_xticks([0, 1]); ax.set_xticklabels(["MI", "REST"])
    ax.set_ylim(0, 100); ax.set_ylabel("Lean %")
    ax.set_title(f"Lean% — Mean with 95% CI ({METRIC_MODE}) {title_suffix}")
    ax.grid(True, axis="y", linestyle="--", alpha=0.4)
    plt.tight_layout(); plt.show()
...
def plot_violin_total(lean_df_subset, title_suffix=""):
    fig, ax = plt.subplots()
    sns.violinplot(
        data=lean_df_subset, x="Class", y="LeanPct", inner=None, cut=0, ax=ax,
        palette={"MI": COL_MI, "REST": COL_REST}
    )
    # mean + CI segment
    for i, cls in enumerate(["MI", "REST"]):
        vals = lean_df_subset.loc[lean_df_subset["Class"] == cls, "LeanPct"].astype(float).values
        m, (lo, hi) = mean_ci(vals)
        ax.vlines(i, lo, hi, color="k", linewidth=3, alpha=0.9)
        ax.plot(i, m, "o", color="white", markersize=10, markeredgecolor="k", markeredgewidth=1.5)
    ax.set_ylim(0, 100); ax.set_ylabel("Lean %")
    ax.set_title(f"Lean% — Violin + Mean + 95% CI ({METRIC_MODE}) {title_suffix}")
...
def plot_ecdf_total(mi_vals, rest_vals, title_suffix=""):
    ...
    ax.set_xlabel("Lean %"); ax.set_ylabel("ECDF")
...
def plot_box_runwise(lean_df_subset):
    ...
    sns.boxplot(
        data=lean_df_subset, x="Class", y="LeanPct", hue="RunID",
        dodge=True, showfliers=False, ax=ax
    )
    ax.set_ylim(0, 100); ax.set_ylabel("Lean %")
    ax.set_title(f"Bar Dynamics — {METRIC_MODE} (Run-wise, single figure)")
    ax.grid(True, axis="y", linestyle="--", alpha=0.4)
    ax.legend(title="Run")
    plt.tight_layout(); plt.show()
...
def plot_dot_std_runwise(mi_by_run, rest_by_run):
    fig, ax = plt.subplots()
    k = len(runs_sorted); off = offsets(k, base=0.18)
    ...
    for j, run in enumerate(runs_sorted):
        ...
        if run not in seen:
            handles.append(Line2D([0], [0], color=col, marker="o", linestyle="-", linewidth=3))
            labels.append(run); seen.add(run)

        for x0, arr in [(0, mi_by_run.get(run, np.array([]))),
            ...
            if DOT_CI_SHOW_DOTS and arr.size:
                ax.scatter(jittered_x(x, len(arr), 0.02), arr, s=18, alpha=0.55,
    ...
    err_label = "SE" if DOT_STD_USE_SE else "SD"
    asym = "asym" if DOT_STD_ASYMMETRIC_SE else "sym"
    ax.set_title(f"Lean% — Mean • ±{err_label} by Run ({asym}) ({METRIC_MODE})")
    ax.grid(True, axis="y", linestyle="--", alpha=0.4)
...
def plot_dot_ci_runwise(mi_by_run, rest_by_run):
    ...
    for j, run in enumerate(runs_sorted):
        ...
        mi_mean, (mi_lo, mi_hi) = mean_ci(mi_vals) if mi_vals.size else (np.nan, (np.nan, np.nan))
        re_mean, (re_lo, re_hi) = mean_ci(re_vals) if re_vals.size else (np.nan, (np.nan, np.nan))

        if DOT_CI_SHOW_DOTS:
            if mi_vals.size:
                ax.scatter(jittered_x(0+off[j], len(mi_vals), 0.02), mi_vals, s=20, alpha=0.5,
                           edgecolor="k", linewidth=0.3, color=col)
            if re_vals.size:
                ax.scatter(jittered_x(1+off[j], len(re_vals), 0.02), re_vals, s=20, alpha=0.5,
                           edgecolor="k", linewidth=0.3, color=col)
    ...
    ax.set_xticks([0,1]); ax.set_xticklabels(["MI","REST"])
    ax.set_ylim(0, 100); ax.set_ylabel("Lean %")
    ax.set_title(f"Lean% — Mean with 95% CI by Run ({METRIC_MODE})")
    ax.grid(True, axis="y", linestyle="--", alpha=0.4)
    ax.legend(handles, labels, title="Run")
    plt.tight_layout(); plt.show()
...
def plot_violin_runwise(lean_df_subset):
    ...
    sns.violinplot(
        data=lean_df_subset, x="Class", y="LeanPct", hue="RunID",
        inner=None, cut=0, dodge=True, ax=ax
    )
    ax.set_ylim(0, 100); ax.set_ylabel("Lean %")
    ax.set_title(f"Lean% — Violin by Run ({METRIC_MODE})")
    ax.grid(True, axis="y", linestyle="--", alpha=0.35)
    ax.legend(title="Run")
    plt.tight_layout(); plt.show()
...
def plot_ecdf_runwise(mi_by_run, rest_by_run):
    ...
    for j, run in enumerate(runs_sorted):
        ...
        if mi_by_run[run].size:
            ...
            ax.step(x_mi, y_mi, where="post", color=col, alpha=0.95,
                    label=f"{run} (MI)" if j==0 else None)
        if rest_by_run[run].size:
            x_re, y_re = ecdf(rest_by_run[run])
            ax.step(x_re, y_re, where="post", color=col, alpha=0.6,
...
# ---- Main plotting dispatch with RUNWISE toggle ----
if plot_style == "forest_runwise":
    ...
    for run in runs:
        for cls in ["MI","REST"]:
            vals = lean_df.loc[(lean_df["RunID"]==run)&(lean_df["Class"]==cls), "LeanPct"].astype(float).values
    ...
    for run in runs:
        for cls in ["MI","REST"]:
            ...
            y_labels.append(f"{run} — {cls}"); y_ticks.append(y); y += 1
        y += 0.3
    ax.set_yticks(y_ticks); ax.set_yticklabels(y_labels)
    ax.set_xlim(0, 100); ax.set_xlabel("Lean % (mean with 95% CI)")
    ax.set_title(f"Lean% — Run-wise forest plot ({METRIC_MODE})")
    ax.grid(True, axis="x", linestyle="--", alpha=0.4)
    plt.tight_layout(); plt.show()

else:
    if not RUNWISE:
        # ---- TOTAL (aggregate) mode: one figure for all runs combined ----
        if plot_style == "box":
                    plot_box_total(mi_vals_total, rest_vals_total, title_suffix="(All Runs)")
        elif plot_style == "dot_std":
                plot_dot_std_total(mi_vals_total, rest_vals_total, title_suffix="(All Runs)")
        elif plot_style == "dot_ci":
            plot_dot_ci_total(mi_vals_total, rest_vals_total, title_suffix="(All Runs)")
        elif plot_style == "violin":
            plot_violin_total(lean_df, title_suffix="(All Runs)")
        elif plot_style == "ecdf":
            plot_ecdf_total(mi_vals_total, rest_vals_total, title_suffix="(All Runs)")
    else:
        # ---- RUNWISE on a single figure (grouped by Class) ----
...
#   "FINAL_WINDOW"     : Lean% computed only over the last LAST_N_SEC of each trial
#   "DERIV"            : % of (time) where d/dt(correct-class prob) > DERIV_EPS
METRIC_MODE      = "MARGIN_AUC"   # change to "FINAL_WINDOW" or "DERIV" to try new metrics
THRESH           = 0.50
