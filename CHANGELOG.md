...
- **Tiagobot grip-after-reach ordering:** The Tiagobot online driver now closes the glove (when `TIAGOBOT_USE_GLOVE=True`) *after* `hold_messages_and_classify` completes, not at the moment MI is detected. The 7-second classification window bounds any A-I motion (~3-5s worst case), so the grip fires reliably at the target. Sequence per MI-correct trial: send letter → hold/classify 7s → close glove (grip) → 2s pre-home fixation → open glove (release) → send HOME → 3s reset. With the glove disabled the trial is just reach → wait → retract.
- **Tiagobot integration (new online driver):** `ExperimentDriver_Online_Tiagobot.py` drives the Tiagobot mobile-arm device (servo + linear actuator, Arduino over USB serial) instead of Harmony. New helper `Utils/tiagobot.py` owns the serial port and exposes `LOCATIONS` (A–I preset dict, calibrated for the Tiagobot servo 9°–69° and actuator analog 350–900), `open_port` (waits up to 30 s for the sketch's `Calibration complete.` banner — the glove driver's 2 s wait is not enough), `send_letter`, and `send_home`. New config keys: `TIAGOBOT_PORT` (machine-local, empty default), `TIAGOBOT_BAUD=9600`, `TIAGOBOT_TRAJECTORY=A..I`, `TIAGOBOT_USE_GLOVE=False` (optional glove toggle). Modified Arduino sketch lives in `tools/tiago_arduino/Final_code.ino`: drops the auto-retract from the GO path (actuator stays at target until next command) and adds a discrete `h\n` HOME command, mirroring Harmony's GO-then-later-HOME pattern in concept. No ACK protocol — fire-and-forget. Tiagobot HOME is sent unconditionally after the classification window (even on early-stop) since the actuator no longer self-retracts. For two-Arduino coexistence (Tiagobot + glove) on Linux, use `/dev/serial/by-id/` paths in `config_local.py` so enumeration is stable across reboots. The existing Harmony drivers are untouched — Tiagobot experiments are run by launching the Tiagobot driver script. Pre-commit `config-py-guard.sh` must be updated locally to include `TIAGOBOT_PORT` in the forbidden-in-`config.py`-diff list.
