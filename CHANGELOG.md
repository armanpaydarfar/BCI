...
- **`environment.yml`: `mne 1.8.0 → 1.9.0` (control env solve fix on Linux).** The pinned export paired the `mne` metapackage `1.8.0` with `mne-base 1.9.0`; conda-forge has since repackaged the `mne 1.8.0` builds so every one now requires `mne-base >=1.8.0,<1.8.1`, making the combination unsolvable from current repodata (`conda env create -f environment.yml` failed on linux-64 with an `mne`/`mne-base` incompatibility). Bumping the metapackage to `1.9.0` matches `mne-base` (the metapackage carries no library code of its own — `import mne` comes from `mne-base`, already `1.9.0`); `mne-lsl`/`mne-qt-browser` pins are unchanged. Caught by machine-sync task 003's first fresh create-test on a Linux control box (the control superset had never been create-validated on any machine — only the perception-only `environment.server.yml` was, on Windows/WSL).
- **Role-based env system + repo hygiene.** Installation now resolves by **role**, selected via `tools/bootstrap_machine.sh --role`: `control` (the Linux operator host — `environment.yml`, conda env `lsl`, the superset incl. CPU perception so single-box dev works) vs `server` (the GPU perception host — perception-only subset plus a CUDA torch build). The server env is a single cross-platform file `environment.server.yml` (conda env `harmony-server`) — a curated spec with no OS-specific conda packages, so conda solves the core per-platform and pip selects the right CUDA torch wheel per platform; the `cu124` wheel tag is the one knob. This supersedes the "single OS-agnostic `environment.yml`" framing in the WS3 bullet below: *that* file is a fully-pinned export carrying linux-64-only conda packages and can't solve on Windows, whereas the curated server spec can. `bootstrap_machine.sh` also drops the retired sibling-`harmony_vlm` clone and `VLM_REPO_DIR` prompt. Separately, removed orphaned/stale tracked files from version control: `Data/` (111 `confusion_matrix_*.csv` from a retired writer), an unused `calib_pose_library_*.npz`, and unused `yolo11n.pt`/`yolov8n.pt`; the in-use `yolo26n.pt` and `poses_with_gaze_*.npz` are untracked-but-kept and now gitignored (`*.pt` / `*.npz`). `environment.server.yml` was create-validated on a Windows/WSL2 RTX 4070 Ti host (conda-forge solve + pip install succeed, `torch.cuda.is_available()` True, all live perception modules import). Since recent conda (24+/26+) runs a ToS pre-flight on the globally configured channels before honoring a conda-forge-only env file, `bootstrap_machine.sh` now warns when `defaults` is configured and prints the conda-forge fix (no Anaconda ToS acceptance required). `pytest` is now declared in **both** env files: the repo's pre-commit pytest hard gate previously no-op'd silently wherever pytest wasn't hand-installed, and a server-role dev box has no `lsl` env to carry it.
