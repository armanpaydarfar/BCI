...
- **WS4 F4: `--vlm-thinking-budget` flag + `VLM_THINKING_BUDGET` config key (Gemini latency knob).** `vlm_service.py` now exposes the Gemini "thinking" budget the in-tree reasoner already plumbed (`perception/intent_reasoner.py`), passing it into `IntentReasoner(thinking_budget=…)`. `0` disables thinking for the lowest latency (collaborator measured the VLM round-trip ~8 s → <2 s on `gemini-2.5-flash`); any int caps it. **Default preserves today's behaviour:** `VLM_THINKING_BUDGET = None` (committed safe default) / flag unset → no `ThinkingConfig` emitted → Gemini's own default budget. `type=int` only coerces a supplied value so the None sentinel survives. Machine-neutral key, committed in `config.py`. The benchmark to decide whether to flip the committed default to `0` is hardware/credentials-in-the-loop (needs a live `GOOGLE_API_KEY` on the GPU host) and is **pending** — the `[VLM] API latency (gemini): N.NNNs` stderr line is the instrument. Ported from the collaborator's shared-`harmony_vlm` change (`origin/main@cfa01b6`), with the default deliberately changed from her `2048` to None to honour WS4's keep-working invariant. Tests: `tests/test_vlm_thinking_budget.py`.
